...
import { DEVDIGEST_ROOT, EXCLUDED_DEVDIGEST_SUBROOTS, MAX_PATH_LENGTH } from './constants.js';
...
export type WriteZoneRefusal = 'outside_devdigest' | 'outside_roots' | 'ci_bundle';
 ...
 *
 * The bundle refusal comes FIRST and applies to every mode, including `save`.
 * `.devdigest/skills/` and `.devdigest/agents/` are generated by an export, and
 * `isInExcludedBundle` below is the same rule the scan applies — so a document
 * cannot be authored into a folder the scan then refuses to hold, which is a
 * document that disappears at the next rescan with nothing said. `AC-107` is a
 * property of the assembled prompt and is enforced on the read side too
 * (`service.ts` → `readCandidate`); this half is what makes the refusal visible
 * to whoever asked for the write.
 */
export function writeZone(
  ...
  if (isInExcludedBundle(path)) return 'ci_bundle';
  if (mode === 'save') {
...
/**
 * Would this path land inside an exported CI bundle — the folder itself
 * included? The WRITE question.
 *
 * SEGMENTS, never a prefix. `.devdigest/skills-lab/x.md` starts with the string
 * `.devdigest/skills` and is a folder nobody excluded, so a `startsWith` gate
 * would quietly take documents `AC-108` keeps. For the same reason
 * `.devdigest/skills.md` is a document called `skills`: an extension makes it a
 * different segment.
 *
 * The family segment is lower-cased for the reason `namedKind` lower-cases its
 * own — a folder called `Skills` must not take a different route to a different
 * answer, and on a case-insensitive filesystem it is the very directory the
 * export wrote. The root segment is compared exactly, matching `rootFor`, which
 * is what decided the path was under `.devdigest` in the first place.
 */
export function isInExcludedBundle(path: string): boolean {
  const segments = segmentsOf(path);
  if (segments[0] !== DEVDIGEST_ROOT) return false;
  const family = segments[1];
  if (family === undefined) return false;
  return (EXCLUDED_DEVDIGEST_SUBROOTS as readonly string[]).includes(family.toLowerCase());
}

/**
 * Is this path bundle CONTENT rather than a document? The READ question, and
 * one segment stricter — which is the whole difference between the two, stated
 * here in code instead of argued twice in prose.
 *
 * A two-segment path is a document called `skills` and stays, the rule
 * `kindForRoot` above already applies. The write question cannot be that
 * strict: it is handed `.devdigest/skills` when someone creates the folder, and
 * allowing that only to refuse every document inside it is a dead end offered
 * to the user.
 */
export function isExcludedBundlePath(path: string): boolean {
  return isInExcludedBundle(path) && segmentsOf(path).length >= 3;
}
