  ...
  try {
    res = await fetch(`${API_BASE}${path}`, {
      ...
      headers: {
        ...
        // FormData is excluded because it carries its own multipart content-type
        // WITH the boundary; overriding that makes the body unparseable server-side.
...
export const api = {
  ...
  /** Multipart POST — the body is FormData, so the header guard above skips it. */
