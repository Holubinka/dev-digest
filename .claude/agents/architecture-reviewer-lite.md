...
description: Eval-harness control variant of architecture-reviewer — identical in every respect except Rule 2 (cite the specific documented rule per finding) is removed. Not for dispatch on real work; exists so evals/agents/architecture-reviewer-lite/ can measure what Rule 2 is worth via eval:repeat + eval:delta against architecture-reviewer. See that eval file's own comment.
...
## Знахідки
| # | severity | introduced / pre-existing | path:line | що саме не тримається |
