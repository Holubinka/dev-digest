...
put into the dispatch; the fifth is how much of it you dispatch at once.
...
- **A command carries its known workaround.** Writing `integration` instead of
  `pnpm exec vitest run .it.test --fileParallelism=false` cost six ~3-minute runs across two
...
| The `*.it.test.ts` suffix, the unit/integration split, and `--fileParallelism=false` on the integration lane | [`TESTING.md`](../../TESTING.md) § *Conventions* |
